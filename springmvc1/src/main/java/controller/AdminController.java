 ...
 * 		2. 관리자가 아닌 경우 : 관리자만 거래 가능합니다. mypage 페이지로 이동
 */
@Controller
...
public class AdminController {
	...
	public ModelAndView list(Integer sort, HttpSession session) {
		...
		case 2: // 아이디의 내림차순
			Collections.sort(list,
			(u1,u2)->u2.getUserid().compareTo(u1.getUserid()));
			break;
		case 3: 
			Collections.sort(list,
		...
		case 4: 
			Collections.sort(list,
			(u1,u2)->u2.getUsername().compareTo(u1.getUsername()));
			break;
		case 5: 
			Collections.sort(list,
		...
		case 6: 
			Collections.sort(list,
			(u1,u2)->u2.getEmail().compareTo(u1.getEmail()));
			break;
		}
